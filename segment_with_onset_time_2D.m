function [x1_all,trigger_time_remained] = segment_with_onset_time_2D(Voltage, timepoint, trigger_time, window)
...
[nchannel,~] = size(Voltage);
itv = timepoint(2)-timepoint(1);
Number_Dots = floor((window(2)-window(1))/itv)-1;
x1_all = zeros(nchannel, Number_Dots,length(trigger_time));
